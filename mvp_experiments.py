...
DEFAULT_THRESHOLD_QUANTILES = [0.50, 0.60, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95]
        ...
        history_rows.append(
            {
                ...
                "valid_prob_mean": float(np.mean(valid_prob)),
                "valid_prob_max": float(np.max(valid_prob)),
            }
        )
...
def evaluate_lstm_horizon_model(
    ...
    if len(valid_select):
        ...
        selected_row = {"quantile": 0.90, "threshold": float(valid_result["pred_prob"].quantile(0.90))}
